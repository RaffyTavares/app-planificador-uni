document.addEventListener('DOMContentLoaded', function() {
            ...
            async function renderMaterias() {
                materias = await fetchData('materias');
...
async function fetchData(table) {
    const response = await fetch(`http://localhost/api.php?table=${table}`);
...
async function updateData(table, id, data) {
    const response = await fetch(`http://localhost/api.php?table=${table}&id=${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
    });
